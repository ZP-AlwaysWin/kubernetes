...
func TestSetDefaults_PodSpec(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			corev1.SetDefaults_PodSpec(tt.args.obj)
			if tt.args.obj.DNSPolicy != tt.dnsPolicy {
				t.Errorf("HostNetwork is %v the default value of DNSPolicy is not %v", tt.args.obj.HostNetwork, tt.dnsPolicy)
			}
		})
	}
}
